...
def test_last_transaction_amount():
    ...
    result = last_transaction_amount.run_transformation(
    ...
    assert user1_transactions.iloc[-1]['amt'] == 300.0  # Last transaction amount for user1

    # Verify user2's transactions
    user2_transactions = result_df[result_df['user_id'] == 'user2']
    assert len(user2_transactions) == 2
    assert user2_transactions.iloc[-1]['amt'] == 500.0  # Last transaction amount for user2
