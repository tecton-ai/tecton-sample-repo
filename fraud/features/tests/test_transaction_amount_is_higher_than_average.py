...
# and a precomputed feature ("amount_mean_24h_1h") as inputs
def test_transaction_amount_is_higher_than_average():
    transaction_request = pd.DataFrame({"amount": [124, 10001, 34235436234]})
    user_transaction_amount_metrics = pd.DataFrame({"amount_mean_24h_1h": [42, 10002, 3435]})

    actual = transaction_amount_is_higher_than_average.run(mock_inputs={"transaction_request": transaction_request, "user_transaction_amount_metrics": user_transaction_amount_metrics})
