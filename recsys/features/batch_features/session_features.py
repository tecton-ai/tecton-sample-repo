...
@batch_feature_view(
    ...
    features=[
        Aggregate(input_column=Field("aid", Int32), function=approx_count_distinct(8), time_window=timedelta(days=1)),
        Aggregate(input_column=Field("aid", Int32), function=approx_count_distinct(8), time_window=timedelta(days=3)),
        Aggregate(input_column=Field("aid", Int32), function=approx_count_distinct(8), time_window=timedelta(days=7))
    ],
    timestamp_field="ts",
